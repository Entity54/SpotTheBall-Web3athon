...
//#endregion
    ...
    function payWinner(string memory gameHash) external onlyGameAuthority { 
            ...
            if (winner.playerEVMAddress!=address(0))
            {
                ...
                else //if (keccak256(bytes(winner.playerChain)) == keccak256(bytes("MOONBEAM")) )
                {
                    // SEND AXLUSDC TO winner.playerSubstrateAddressString in Moonbeam (Native) using his winner.playerHex
